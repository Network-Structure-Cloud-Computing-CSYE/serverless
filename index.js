...
const secretName = process.env["EMAIL_SECRET"];
...
exports.handler = async (event) => {
    try {
        ...
        console.log('secretObj =', secretObject)
        const domain = secretObject['domain'];
        const apiKey = secretObject['EMAIL_API_KEY'];
        // console.log("************domain=***************** " , domain)
        // console.log("************EMAIL_API_KEY=*****************  " , apiKey)
        ...
        // console.log("************Before defining Transporter=*****************  ")
        const transporter = nodemailer.createTransport(mailGun(auth));
        console.log("************After defining Transporter=*****************  ")
        try{
            ...
            console.log("*************INSIDE CATCH ********************",DYNAMO_TABLE_NAME )
            let mailOptions;
            const dynamoDBParams = {
                TableName: DYNAMO_TABLE_NAME, // Replace with your DynamoDB table name
                Item: {
            ...
            if(error.code === "UNSUPPORTED_PROTOCOL"){ //error.code === 'ERR_BAD_REQUEST'
                ...
                mailOptions = {
                    ...
                    to: email,
                    subject: 'Submission status',
                    text: `Submission download was unsuccessful as ${error.message}`,
                };
                dynamoDBParams.Item.body.S = `Submission download was unsuccessful as ${error.message}, and failure mail sent to ${email}`
